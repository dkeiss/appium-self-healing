...
public class LocatorHealer {
    ...
    public HealingResult heal(FailureContext context) {
        ...
        try {
            ...
            By healedLocator = constructLocator(response.locatorMethod(), response.locatorValue());
    ...
    private By constructLocator(String method, String value) throws Exception {
        return switch (method.toLowerCase()) {
            ...
            default -> {
                // Fallback: try AppiumBy first, then By
                try {
                    Class<?> appiumBy = Class.forName("io.appium.java_client.AppiumBy");
    ...
    private String stripResourceIdPrefix(String value) {
        if (value != null && value.contains(":id/")) {
            String stripped = value.substring(value.indexOf(":id/") + 4);
            log.debug("Stripped resource-id prefix: {} → {}", value, stripped);
            return stripped;
        }
        return value;
    }
